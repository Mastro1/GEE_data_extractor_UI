...
export default function App() {
  ...
  const [runState, setRunState] = useState<{
    ...
    message: string;
    artifacts?: RunExtractionResponse['artifacts'];
  }>(() => ({
    status: 'idle',
    message: '',
  }));
  ...
  const handleRunExtraction = useCallback(async () => {
    ...
    try {
      ...
      if (result.status === 'success' && result.summary) {
        ...
        setRunState({
          status: 'success',
          message: `Extraction executed for ${collection}. ${images_found} image(s) filtered. Export method: ${export_method}. ${maskMessage}`,
